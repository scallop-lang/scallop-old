...
#[structopt(name = "sum3-decouple-ce")]
struct Options {
...
impl<const TRK: usize, const TEK: usize> Trainer<TRK, TEK> {
  fn new(opt: Options) -> Result<Self> {
    ...
    let out_dim = num_digits_per_task * 9 + 1;
    ...
    let file_name = format!("{}/sum_3_decouple_ce_trk_{}_tek_{}_lr_{}_bs_{}_rs_{}.log", log_dir, TRK, TEK, learning_rate, batch_size, opt.seed);
  ...
  fn run_batch<const K: usize>(&mut self, phase: Phase, bimages: Tensor, blabels: Tensor) -> (usize, f64, f64) {
    ...
    let (data, y_gt_arr) : (Vec<_>, Vec<_>) = (0..batch_num).map(|i| {
      ...
      let y_index : i64 = first_y + second_y + third_y;
      ...
      ((first, second, third, y_index), y_slice)
    }).unzip();
    ...
    let tasks = data.into_iter().map(|(first, second, third, y_index)| {
      let mut prog = Sum3::<DiffTopKProbProofs<K>>::new();
      ...
      let results = prog.sum_3().complete().into_iter().into_iter().collect::<Vec<_>>();
