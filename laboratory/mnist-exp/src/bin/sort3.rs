...
impl<const K: usize> Trainer<K> {
  fn new(opt: Options) -> Result<Self> {
    ...
    let file_name = format!("{}/sort_3_k_{}_lr_{}_bs_{}_rs_{}.log", log_dir, K, learning_rate, batch_size, opt.seed);
  ...
  fn run_batch(&mut self, phase: Phase, bimages: Tensor, blabels: Tensor) -> (usize, f64, f64) {
    ...
    let (data, y_gt_arr) : (Vec<_>, Vec<_>) = (0..batch_num).map(|i| {
      ...
      let c1 : i64 = blabels.i(i * self.num_digits_per_task).into();
      let c2 : i64 = blabels.i(i * self.num_digits_per_task + 1).into();
      let c3 : i64 = blabels.i(i * self.num_digits_per_task + 2).into();

      // Compute ground truth
      let y_index : i64 = if c1 <= c2 && c2 <= c3 {
        0
      } else if c1 <= c3 && c3 < c2 {
        1
      } else if c2 < c1 && c1 <= c3 {
    ...
    let tasks = data.into_iter().map(|(first, second, third)| {
      let mut prog = Sort3::<DiffTopKProbProofs<K>>::new();
      ...
      let results = prog.sort_3().complete().into_iter().into_iter().collect::<Vec<_>>();
